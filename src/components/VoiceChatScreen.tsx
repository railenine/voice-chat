...
export const VoiceChatScreen: React.FC<VoiceChatScreenProps> = ({ nickname, roomId }) => {
  ...
  return (
    <div className="min-h-screen animated-gradient flex flex-col">
      {/* Header */}
      <header className="p-3 sm:p-4 border-b border-white/10 backdrop-blur-sm">
        <div className="max-w-2xl mx-auto flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-gradient-to-r from-blue-500 to-cyan-500 flex items-center justify-center shadow-lg shadow-blue-500/30 flex-shrink-0">
          ...
          <button
            ...
            className="px-3 sm:px-4 py-2 bg-white/10 hover:bg-white/20 text-white rounded-lg transition-all text-xs sm:text-sm border border-white/10 flex-shrink-0"
          >
      ...
      {/* Share Panel */}
      {showShare && (
        <div className="bg-white/5 border-b border-white/10 p-3 sm:p-4 animate-fade-in">
        ...
        <div className="max-w-2xl w-full">
          ...
          {!isConnected && !error && (
            <div className="text-center mb-6 sm:mb-8">
              <div className="inline-flex items-center gap-2 sm:gap-3 px-4 sm:px-6 py-2 sm:py-3 bg-yellow-500/10 border border-yellow-500/20 rounded-full">
          ...
          {error && (
            <div className="text-center mb-6 sm:mb-8">
              <div className="inline-flex items-center gap-2 sm:gap-3 px-4 sm:px-6 py-2 sm:py-3 bg-red-500/10 border border-red-500/20 rounded-full">
      ...
      {/* Controls */}
      <footer className="p-4 sm:p-6 border-t border-white/10 backdrop-blur-sm">
        <div className="max-w-2xl mx-auto flex items-center justify-center gap-4 sm:gap-6">
          ...
          <button
            ...
            className="w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-red-500/20 hover:bg-red-500 border-2 border-red-500/30 hover:border-red-500 flex items-center justify-center transition-all active:scale-90"
            title="Выйти"
          >
          ...
          {!isMuted && isConnected && (
            <div className="flex items-center justify-center gap-1 h-5">
              {[...Array(5)].map((_, i) => (
                <div
                  key={i}
                  className="w-1 bg-cyan-400 rounded-full sound-wave-bar"
                  style={{ height: '4px' }}
                />
              ))}
            </div>
          )}
          <p className={`text-xs sm:text-sm font-medium ${isMuted ? 'text-red-400' : 'text-cyan-400'}`}>
